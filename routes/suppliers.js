exports.showSuppliers = function(req, res, next){
	req.getConnection(function(error, connection){
		...
		connection.query('SELECT Name  FROM suppliers LIMIT 0 , 5', [], function(error, results) {
			if (error) return next(error);
			console.log(results);
			connection.query('SELECT Name  FROM suppliers LIMIT 0 , 5', [], function(error, results1) {
				if (error) return next(error);
...
exports.get = function(req, res, next){
	...
	req.getConnection(function(err, connection){
		connection.query('SELECT Name  FROM suppliers WHERE Id = ?', [id], function(err,rows){
			if(err){
